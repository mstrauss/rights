...
module Rights
  ...
  class CLI
    ...
    def banner
      ...
      "Usage: #{Pathname($0).basename} [global options]"
    end
    ...
    def execute( arguments=[] )
      ...
      STDIN.each_line do |cmd|
        command, messages = Command.parse( cmd, options )
        unless command
          puts red messages
          puts green Command.help
        else
          begin
            status = command.execute
            @stdout.puts status.messages if status.messages
            puts status.code
